class GigsController < ApplicationController
  ...
  def search
  @gigs = Gig.where("artist_name LIKE ? or gig_genre LIKE ? or gig_venue LIKE ? or gig_when LIKE ?", "%" + params[:q] + "%", "%" + params[:q] + "%", "%" + params[:q] + "%", "%" + params[:q] + "%")
  end
  ...
  private
    ...
    def gig_params
      params.require(:gig).permit(:user_id, :artist_name, :gig_genre, :gig_venue, :gig_when, :gig_tickets, :gig_comments)
    end
end
